...
int main()
{
	...
	HINTERNET hRequest = WinHttpOpenRequest(hConnect, L"GET", L"/?v=9&encoding=json", NULL, L"gateway.discord.gg", WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
	...
	char identifyTemplate[] = "{ \"op\": 2, \"d\": { \"token\": \"%s\", \"intents\": 33280, \"properties\": { \"os\": \"windows\","
							  "\"browser\": \"winhttp\","
							  "\"device\": \"winhttp\"}}}";
	...
	for(;;){
		char inLoopJSON[3000];
		Recieved = WinHttpWebSocketReceive(hWebsocket, inLoopJSON, 3000, &beenRead, &bufferType);
		if (Recieved == NO_ERROR) {
			inLoopJSON[beenRead] = 0;
			printf("result of in main thread recieve: %s\n", inLoopJSON);
			// regex_t regexCompiled;
			// regmatch_t groupArray[2];
			// char regexString[] = "\"content\":\"(.*?)\",";
			// int comp = regcomp(&regexCompiled, regexString, REG_EXTENDED);
			// printf("%d\n",comp);
			// HANDLEERR(comp == 0, "regcomp");
			// int match = regexec(&regexCompiled, inLoopJSON, 2, groupArray, 0);
			if (strstr(inLoopJSON, "!ping") != NULL) {
